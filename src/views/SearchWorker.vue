<template>
    <main class="bg-blue-100 text-[#003D80] px-4 pt-4 pb-7 flex flex-col min-h-screen items-center">
      <div>
        <h1 class="sm:text-[1.2rem] md:text-[1.4rem] font-bold text-center py-4 leading-normal">
        ...
        <div class="overflow-hidden bg-white shadow sm:rounded-lg w-auto" v-if="resultado">
          <div class="px-4 py-5 sm:px-6">
            ...
            <p class="mt-1 max-w-2xl text-sm text-blue-900">
              Antes de continuar, asegúrate que la información señalada no presenta errores.
            </p>
          </div>
  ...
  async function searchByRut(rutSinDV) {
    try {
      ...
      const res = await searchWorkerByRut(rutSinDV.trim());
      if (res) {
        resultado.value = res;
        WORKER.value = fromObject2Array(res);
        searching.value = false;
      } else {
        throw new Error(`No se encontró el RUT ${rutSinDV}`);
      }
    } catch (error) {
      searching.value = false;
